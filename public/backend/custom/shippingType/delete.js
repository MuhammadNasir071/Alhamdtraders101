api['deleteShippingType'] = ajax_path + '/admin/shippingtype/:id';

$(function(){
    $('body').on('click', '.delete-shipping_type', function() {
        deleteShippingType($(this));
    });
    function deleteShippingType(data) {
        if ($(data).attr("data-id")) {
            let remove_id = data.data("id");
            let url = api["deleteShippingType"];
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.responseCode === 200) {
                                toastr.success('Shipping type deleted successfully');
